...
def load_csv(path: str):
    ...
    with open(path, newline="") as f:
        ...
        for row in reader:
            ...
            Ns.append(int(row["N"]))
            blocks.append(int(row["blocks"]))
            means.append(np.mean(filtered))
            stds.append(np.std(filtered, ddof=1) if len(filtered) > 1 else 0.0)

    # sort by N
    order = np.lexsort((blocks, Ns))

    Ns = np.array(Ns)[order]
    blocks = np.array(blocks)[order]
    means = np.array(means)[order]
    stds = np.array(stds)[order]
    removed = len(samples) - len(filtered)
    if removed > 0:
        print(f"N={row['N']}, blocks={row['blocks']}: removed {removed} outlier(s)")


    return Ns, blocks, means, stds


def plot_linear(Ns, blocks, means, stds, outpath):
...
def plot_speedup(Ns, blocks, means, outpath):
    plt.figure(figsize=(8, 6))
...
def main():
    ...
    parser.add_argument("--input", default="blocked_results.csv")
    ...
    Ns, blocks, means, stds = load_csv(args.input)

    if len(Ns) == 0:
        sys.exit("No data found in CSV.")

    plot_linear(Ns, blocks, means, stds, Path(args.outdir) / "runtime_linear.png")

    plot_speedup(Ns, blocks, means, Path(args.outdir) / "runtime_speedup.png")
